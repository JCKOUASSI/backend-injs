...
> bornés par la lecture d'onglet. Le **[LOT 36]** achève l'écran de gestion
> des comptes **`Users`** (ouvert au LOT 2 à 89 % de lignes) : **37 nouveaux
> tests**, portés à **99,8 % de lignes / 100 % de fonctions / 98,1 % de
> branches / 54 tests au total** — présentation (badges, initiales, replis,
> secrétariat rattaché), permissions dégradées, création de comptes
> personnels/étudiants/enseignants, **création d'un secrétariat à la volée**
...
> Le **[LOT 38a]** ouvre le plus gros écran restant, **`Statistiques.jsx`**
> (5 491 lignes, 8 onglets) : **44 nouveaux tests purs**, sans aucune
> modification de la page (seul le mock d'API de test accepte désormais des
> réponses de route asynchrones pour figer les états de chargement), et fait
> passer le fichier de **28 % à 64 % de lignes**. Sont verrouillés : les
> sections API par onglet, les états de chargement/erreur (plein écran et
> bandeau non bloquant, réessai), les filtres d'en-tête et de période, les
...
> par grade. **Un écart avait été révélé (§10.15), corrigé depuis au LOT 44** :
> lors de la fusion de deux lignes Point global d'un même grade, la clé
> erronée `effectif_étudiants` (au lieu de `effectif_auditeurs`) empêchait la
> sommation des effectifs ; les tests `[écart]` documentaient alors le
> comportement de l'époque, ce sont aujourd'hui des régressions.
> Le **[LOT 38c]** verrouille le **workflow des rapports périodiques**
> (`components/RapportsWorkflowPanel.jsx`, rendu dans la vue « workflow » de
> l'onglet Rapports) par **46 nouveaux tests purs** dans un fichier de test
> colocalisé dédié, sans toucher au composant : chargement et périmètre
> (formation / secrétariat / période), droits par rôle (auditeur,
> secrétariat, admin), sélection et détail (KPI, acteurs, dates), les 4
> transitions du workflow (soumettre / valider / rejeter avec motif
> obligatoire / publier, parcours complet), la génération, la modification
...
> Le **[LOT 38e]** poursuit le maillage fin de `Statistiques.jsx` par **29
> nouveaux tests purs**, toujours rendus par la page complète (montage
> authentifié, aucune modification de source) : navigation **ensemble ↔
> détail de la Vue d'ensemble** (les 5 zones cliquables de la synthèse,
> panneaux de section Surveillance vide/remplie, Chiffres clés, Pédagogique,
> Opérationnel, boutons de bascule vers les onglets Alertes/Pédagogique/
> Administratif, libellé secondaire de période) ; **périmètres pédagogiques**
> (`buildPedagogieEntries` sur les 3 dimensions, tableaux formation/secrétariat
> et barres grades cliquables y compris au survol, `PedagogiqueDetailPanel` :
> tags FOR/GRD/SEC, écart vs global en points, grades liés, absents notoires
> filtrés par secrétariat, passage vers l'onglet Secrétariats avec filtre
> positionné et ouverture automatique du détail puis réinitialisation, cas
> d'un taux absent), états sans répartition ; **détail mensuel de
> l'historique** (ordre inverse de la sidebar, variation conventionnelle
> +100 % au premier mois et +20 % en mois suivant, fenêtre glissante 1/2/3
> mois, mois sans données à −100 %, KPI strip des absents notoires) ;
> **secrétariats** (agrégats du comparatif, 17 colonnes et formats —
> responsable manquant replié sur « — », VH, infobulle de pourcentage,
> mention notoires conditionnelle, clic sur une ligne du tableau comme sur la
> sidebar, `SecretariatDetailPanel` complet avec donut des statuts, formations
> et grades rattachés, cartes AN, replis « Aucune formation/Aucun grade/
> Données non disponibles », échec de chargement, chapeau du filtre
> formation) ; boutons Actualiser des quatre onglets. Le fichier passe de
> 87 % à **95,7 % de lignes** (86 % de fonctions). Seuls subsistent trois
> replis défensifs inatteignables via le parcours réel (section inconnue,
> spinner doublon que le parent ne rend jamais, périmètre introuvable).
> Aucun écart produit révélé.
> Le **[LOT 39]** ouvre le backlog des grands écrans restants par **40
> nouveaux tests purs** sur `Modules.jsx` (CRUD pédagogique), jusque-là à
> 37 % de lignes et 6 % de fonctions : liste et badges des 4 statuts,
> recherche débouncée, filtres et modes de date, pagination, secours de
> chargement des formations, permissions par rôle (ADMIN, ÉTUDIANT,
> DIRECTION, flag de rôle), création/édition d'un module (catalogue
> référentiel, cascade site/bâtiment/salle, champs libres de repli,
> erreurs de validation), création de formation, suppression et archivage
> à triple confirmation, fermeture des modales par la croix, Annuler et le
> clic sur l'overlay. Le fichier atteint **100 % de lignes et de fonctions**
> (89 % de branches) ; aucun écart produit révélé.
> Le **[LOT 40]** verrouille l'écran-liste **`Maquettes.jsx`** (maquettes
> pédagogiques LMD), qui n'avait aucun test, par **20 nouveaux tests purs** :
> liste, badges de statut, filtre, référentiels conditionnels aux
> habilitations, création avec IDs typés, permissions `canActScolarite`,
> workflow valider / activer / archiver / cloner sous confirmation avec les
> trois niveaux d'erreur et la désactivation pendant le traitement. Le
> fichier atteint **100 % de lignes et de fonctions** (93 % de branches) ;
> aucun écart produit. Ce lot fiabilise aussi un test 38a qui pouvait
...
> produit révélé.
> Le **[LOT 42]** parachève le tableau de bord transverse **`Dashboard.jsx`**
> (812 lignes), qui était le dernier écran fonctionnel majeur en couverture
> partielle (75 % de lignes, 43 % de fonctions) : **27 nouveaux tests purs**
> (fichier porté de 7 à **34 tests**) couvrent les KPI d'en-tête, les cartes
> Capacité du jour / Exécution live / Qualité, le tableau des modules débutés
> (replis, seuils de couleur, liens), les prochaines séances et les derniers
> pointages, la carte Secrétariats (filtrage et état de chargement), le jour
> de référence, la période volume horaire (preset, période future), les trois
> variantes de rôles (DIRECTION, ENCADRANT, SECRETARIAT), les états vides et
> les formes de réponse défensives. L'écran atteint **99,7 % de lignes,
> 100 % de fonctions et 98 % de branches** ; les quelques branches
> résiduelles sont inatteignables par l'UI ou liées à deux écarts SIGNALÉS
> et non corrigés (**§10.16** : rafraîchissement de visibilité non silencieux
> par incohérence de contrat booléen/objet ; **§10.17** : `presence_period`
...
> Le **[LOT 44]**, sur feu vert explicite, corrige l'écart **§10.15** dans le
> Bilan FAC (`BilanFACPointGlobalTable`, `Statistiques.jsx`) : la fusion des
> lignes d'un même grade additionnait une clé fantôme `effectif_étudiants` au
> lieu d'`effectif_auditeurs`, d'où un effectif figé à la première ligne en
> cas de doublon. Le correctif cumule désormais l'effectif explicitement,
> **après** avoir lu les poids servant aux moyennes pondérées (un simple
> remplacement de clé dans la liste des sommes aurait compté la nouvelle
> ligne deux fois dans les poids et biaisé les taux — vérifié par un test à
...
| `src/pages/Modules.test.jsx` | page | **Nettoyage des filtres obsolètes (2 tests, LOT 6)** : à l'arrivée des référentiels, un filtre d'URL absent des options (`grade=999`) est écarté, la liste est rechargée sans lui (un filtre valide comme `statut` est conservé) et un toast « Filtre(s) ignoré(s) » informe l'utilisateur ; cas contraire (filtres tous valides), aucune alerte. Couvre l'effet `referentielsData` dont les dépendances faisaient un faux positif ESLint (§10.2). **LOT 39 (couverture fonctionnelle complète, +40 tests, sans toucher à la page), `Modules.jsx` porté à 100 % de lignes et de fonctions** : rendu de la liste (9 colonnes, les 4 badges de statut `badge-planifiee/en-cours/terminee/suspendue` + badge générique pour un statut inconnu, dates `formatDate` JJ/MM/AAAA, replis `—`/`-` des grade/secrétariat/groupe/dates, compteur « N module(s) », lien Détail), état vide « Aucun module trouvé », spinner figé par promesse pilotée, message d'erreur de chargement ; **recherche** avec debounce 400 ms et retour page 1 après pagination ; **filtres** statut / secrétariat / vague / grade / groupe, modes de date (`today`, `all`, `date` avec apparition du sélecteur et paramètre `date`), sélecteur de secrétariat masqué pour les rôles périmètre, mode de date invalide corrigé silencieusement, **pagination** (51 résultats sur 2 pages), liste de secours `/formations/list/?page_size=500` avec déduplication des doublons d'id quand le référentiel n'a pas de formations réelles ; **permissions par rôle** (ADMIN tout voir, AUDITEur en lecture seule, DIRECTION archivage seul, module déjà archivé sans bouton, flag `role_context.can_archive_modules`) ; **création de module** (POST `/formations/<id>/modules/`, champ formation pré-rempli après création d'une formation, intitulé en liste dérivée du référentiel avec pré-remplissage du volume horaire et filtrage par `formation_ids`, cascade site → bâtiment (reset) → salle (reset), champs texte de repli quand les référentiels sont vides, dates nulles et volume 0 par défaut, erreurs de validation formatées `champ: raison` et erreur générique) ; **édition** (PATCH, pré-remplissage des dates tranchées, formation non modifiable) ; fermetures par Annuler / croix / clic sur l'overlay pour les deux modales ; **création de formation** (POST `/formations/` en select d'intitulés ou saisie libre, nouvelle formation ajoutée et présélectionnée dans la modale module, toast, cas d'erreur) ; **suppression** (`ConfirmModal` avec le nom du module et le caractère définitif, DELETE, toast, rechargement, annulation, échecs avec/sans détail) ; **archivage** (`TripleConfirmModal` : Continuer / Je comprends / Archiver définitivement, POST `…/archive/`, annulation initiale, échecs avec/sans détail). |
...
| `pages/Statistiques.jsx` (LOT 38a socle, 38b Bilans, 38e maillage fin) | **96 %** | **96 %** | **86 %** | **74 %** |
...
> **100 % de lignes et de fonctions**, le LOT 41 achève `MonEspace.jsx`
> (74 % → **100 % de lignes, fonctions et branches**, dossier
> `pages/scolarite/` porté à 96,6 % de lignes et 94,2 % de fonctions), et
...
> Le **LOT 36 achève la gestion des comptes `Users`** (écran ouvert au
> LOT 2 : de 89 % de lignes à **99,8 %**, de 49 % de fonctions à **100 %**) :
> le **secrétariat créé à la volée** lors de la création d'un compte
> SECRETARIAT est vérifié de bout en bout (POST de la structure avec
...
> Le **LOT 38a ouvre le plus gros écran restant, `Statistiques.jsx`**
> (5 491 lignes, 8 onglets) : il passe de **28 % à 64 % de lignes** par
> **+44 tests purs** sans modifier la page (seul le mock d'API de test gagne
> le support des routes asynchrones, pour figer les états de chargement).
> Sont désormais verrouillés : les sections API par onglet, les états de
> chargement/d'erreur (plein écran, réessai, bandeau non bloquant), les
> filtres d'en-tête et de période (y compris période future et absence de
> séance), les états vides et rendus minimaux des 8 onglets, le cycle des
...
- trois endroits de `Users.jsx` appliquent systématiquement
  `.replaceAll('CPFAE', 'INJS')` sur les textes issus du backend : les
  **libellés de rôles** (`roleLabel`, utilisé dans les badges et les
...
### 10.15 ~~Corrigé au LOT 44~~ — `Statistiques` / Bilan FAC : la fusion des lignes Point global ne sommait pas les effectifs auditeurs (clé erronée `effectif_étudiants`)

> **Corrigé au LOT 44 (feu vert explicite).** La clé fantôme
> `effectif_étudiants` a été retirée de `sumKeys` ; l'effectif auditeurs est
> désormais cumulé explicitement (`existing.effectif_auditeurs = n1 + n2`)
> **après** lecture des poids `n1`/`n2` des moyennes pondérées — un simple
> remplacement de clé dans la liste des sommes aurait fait sommer
> `effectif_auditeurs` *avant* la relecture de `n1`, comptant la ligne
> fusionnée deux fois et biaisant les taux (démontré par la simulation et
> verrouillé par le test à taux distincts : 80 %/100 % → 91,11 %). Le test
> `[écart]` est devenu une régression (effectif 45 cohérent avec TOTAL) et le
> test positif de fusion vérifie aussi l'effectif. Analyse initiale
...
**Correctif appliqué au LOT 44 (feu vert explicite)** — la solution d'une
simple substitution de clé dans `sumKeys` s'est révélée insuffisante : la
boucle de sommes s'exécute *avant* la relecture de `existing.effectif_auditeurs`
servant de poids aux moyennes, ce qui aurait compté la ligne fusionnée deux
fois (effectif correct à 45, mais taux biaisés, ex. 87,14 % au lieu de
91,11 %). Le correctif retire donc la clé erronée de `sumKeys` et cumule
l'effectif explicitement après lecture des poids. Les deux tests mentionnés
ont été adaptés : le test **`[écart]`** est devenu une régression qui exige
`45` sur la ligne grade (cohérent avec le TOTAL) **et** un taux pondéré
exact (`91,11 %` avec des taux de ligne distincts), et le test positif de
fusion vérifie désormais aussi l'effectif.

Avant le LOT 44, deux tests figuraient le comportement RÉEL : le test positif
vérifiait les sommes qui fonctionnaient déjà (groupes, absents, VH, TOTAL
backend) et le test **`[écart]`** constatait l'effectif figé à 20 face au
TOTAL 45.
...
fonction de rafraîchissement avec un **booléen** :

```js
// useVisibilityPolling.js
const tick = () => { if (document.visibilityState === 'visible') callbackRef.current(true) }
```

Les deux consommateurs n'ont pourtant pas la même signature :

```js
// ModuleDetail.jsx — convention du hook : un booléen positionnel
const refreshPresences = useCallback(async (silent = true) => { … }, …)

// Dashboard.jsx — attend un OBJET d'options
const loadDashboardData = useCallback(async ({ silent = false } = {}) => { … })
```

Quand le hook transmet `true`, la déstructuration `{ silent = false }`
s'applique à une valeur booléenne (qui n'a pas de propriété `silent`) : le
défaut `false` s'applique. Conséquence : sur `Dashboard`, le rechargement
déclenché par un retour sur l'onglet (`visibilitychange`) ou par le polling
passe systématiquement par la branche non silencieuse, qui réécrit `error` et
`loading` (lignes ~93-103). Concrètement, un **échec passager du
rafraîchissement de fond fait apparaître le bandeau « Certaines données n'ont
pas pu être chargées »**, là où un rafraîchissement réellement silencieux
n'aurait pas touché à l'affichage ; inversement, un rafraîchissement réussi
peut effacer un message d'erreur légitimement affiché.

**Sévérité faible** : les données sont quand même rechargées, aucun appel
supplémentaire n'est perdu, et `setLoading(false)` après le premier
chargement n'a pas d'effet visible ; seul le comportement d'erreur du
rafraîchissement de fond est incohérent avec l'intention documentée par le
paramètre `silent`.

Le test **`[écart] le rafraîchissement au retour d'onglet n'est pas
silencieux et affiche l'échec de fond (§10.16)** fige ce comportement
RÉEL (première réponse saine, échec au `visibilitychange` → bandeau affiché).
**Correctif proposé (non appliqué, en attente de feu vert utilisateur)** :
harmoniser le contrat — soit appeler `callback({ silent: true })` dans le
hook et adapter `ModuleDetail.refreshPresences`, soit faire accepter à
`Dashboard.loadDashboardData` un booléen comme son voisin. En cas de feu
vert, le test `[écart]` deviendra une régression exigeant que l'échec d'un
rafraîchissement de fond n'affiche pas le bandeau ; la branche `silent`
...
1. **`presence_period` non validé.** `readDashboardFilters`
   (`utils/listFilters.js`) transmet tel quel le paramètre d'URL
   `presence_period`. Avec une valeur inconnue (URL trafiquée ou lien
   erroné, ex. `?presence_period=bizarre`), si les *valeurs* sont
   sauvegardées par un filet (`periodStats[presencePeriod] ||
   periodStats.jour`, ~L160), les *libellés* ne le sont pas : le rendu lève
   sur `periodLabels[presencePeriod].toUpperCase()` (~Dashboard.jsx:390),
   puis `.toLowerCase()` (~L423), et le bloc présences afficherait
   « Présences undefined ». Aucun `ErrorBoundary` n'encadre la page :
   **l'écran devient entièrement blanc**. Le test
   **`[écart] une presence_period inconnue dans l'URL fait planter le
   rendu (§10.17)`**, qui encapsule la page dans une sonde de rendu de test,
   fige l'exception (`Cannot read properties of undefined (reading
   'toUpperCase')`).

2. **Date de référence vidée.** Lorsque l'utilisateur efface complètement le
   champ « Jour spécifique », `referenceDate` devient `''` : le bloc
   présences et la liste retombent bien sur le jour courant
   (`seance_en_cours=true`, libellé « du jour »), mais le sous-titre de
   bienvenue construit `new Date('T00:00:00')` et affiche
   **« • Référence: Invalid Date »** (~Dashboard.jsx L278). Le test
   **`[écart] vider « Jour spécifique » replace sur le jour mais affiche
   « Référence: Invalid Date » (§10.17)`** fige ce comportement.

**Sévérité :** (1) moyenne mais uniquement déclenchable par une URL
manuelle erronée (l'interface ne produit jamais de valeur inconnue via les
boutons) ; (2) purement cosmétique, déclenchée par un effacement complet du
champ date.

**Correctif proposé (non appliqué, en attente de feu vert utilisateur)** :
valider/normaliser `presence_period` à la lecture (`['jour','semaine',
'mois','annee'].includes(...) || 'jour'`), ce qui couvrira aussi les
libellés et le fallback `periodFields[presencePeriod] || periodFields.jour`
(~L566) aujourd'hui inatteignable (le crash se produit avant) ; et
masquer la mention « Référence » / remplacer par la date du jour quand
`referenceDate` est vide ou invalide. En cas de feu vert, les deux tests
`[écart]` deviendront des régressions (page rendue avec repli « jour » ;
sous-titre absent ou cohérent).
...
- le **LOT 36 achève l'écran de gestion des comptes `Users.jsx`**
  (**54 tests au total dont 37 nouveaux, 99,8 % de lignes, 100 % de
  fonctions, 98,1 % de branches**, lot de tests purs, fichier produit non
  modifié) : présentation (replis, initiales de secours, badges par rôle,
  remplacement CPFAE→INJS des libellés), permissions dégradées
  (`role_context` vide, gestionnaire sans rôle maniable / sans
  `staff_filter_roles`, repli sur un onglet non personnel quand c'est le
  seul disponible), création de comptes tous profils (payloads exacts,
  compte enseignant et son libellé de badge), rattachement à un secrétariat
  existant, **création d'un secrétariat à la volée** (POST structure avec
  `responsable`, PATCH de rattachement dans le bon ordre, type vide,
  cache React Query mis à jour et vérifié à la réouverture, pas de création
  sans nom, réinitialisation au changement de rôle, bloc masqué pour un
...
- le **LOT 38a est un lot de tests purs qui ouvre `Statistiques.jsx`**
  (5 491 lignes, 8 onglets, auparavant 28 % de lignes pour 8 tests) : **44
  tests supplémentaires (52 au total)** qui portent le fichier à **64,2 % de
  lignes / 52 % de fonctions / 58,7 % de branches**. Sont couverts : la
  présence des 8 onglets et les sections API dédiées par onglet (`TAB_SECTIONS`,
  endpoint `/statistiques/secretariats/`, entrée directe `?rbView=workflow`),
  les états de chargement (spinner initial figé par promesse, « Actualisation… »)
  et d'erreur (détail backend, message générique, écran plein + Réessayer qui
  répare, bandeau non bloquant après chargement, erreur du Point Journalier),
  les filtres d'en-tête (formation peuplée par la méta et propagée, effacement,
  horodatage, sélecteur secrétariat activé/désactivé pour un encadrant), le
  filtre de période (auto-apply, période future + retour au trimestre courant,
  badge, message sans séance comptabilisable), les états vides et rendus
  minimaux des 8 onglets (dont navigation mensuelle de l'historique et
  comparatif + détail secrétariat), le cycle des seuils d'alertes (invite
  d'init INJS, masquage pour rôle non validant, POST puis re-fetch, édition +
  PUT du corps, échecs en `window.alert`, alerte déclenchée et synthèse) et
  les exports Point Journalier / Bilans (désactivés à vide, blob XLSX annuel et
  PDF du tableau sélectionné avec `jour`/`formation_id`/`categorie`, état
  d'export puis réactivation, échecs traités). Seule concession hors page :
  le mock d'API de test (`src/test/utils/mockApi.js`) accepte désormais les
  fonctions de route asynchrones (await de la réponse), amélioration
  défensive sans effet sur les tests existants. Aucun écart §10 révélé ;
...
1. ~~Moteur de listes + décision pédagogique~~ (LOT 1), ~~liste *serveur*
   ...
   (LOT 3) ; la branche `Users` « création d'un secrétariat à la volée »
   est **couverte au LOT 36**. Les onglets/exports/widgets internes de
   `Statistiques` sont **couverts pour l'essentiel du socle au LOT 38a**
   (sous-lots d'approfondissement FAC / détails bilans / workflow en
   backlog, point 3). Les bugs
...
3. flux critiques par rôle : ~~candidatures~~ (LOT 9), ~~admissions/décisions~~
   ...
   **les quatre écrans Finance transverse ont désormais leur test dédié**,
   ...
   33, les **référentiels de formation `Referentiels.jsx` sont
   ...
   ont été maillés au sous-lot 38e ; l'écart §10.15 sur la fusion Point
   global (clé `effectif_auditeurs`) est **corrigé au LOT 44**. Les
...
4. ~~écarts encore ouverts, dans des lots dédiés~~ **: tous soldés au LOT 46
   ...
   - ~~§10.15 Bilan FAC (Point global) : effectifs auditeurs non sommés lors
     ...
     l'effectif après lecture des poids des moyennes ; le test `[écart]` du
     LOT 38b est devenu une régression qui verrouille aussi la pondération) ;
