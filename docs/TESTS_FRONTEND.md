...
> matrice FINANCE/lecture seule et période partagée via le shell).
...
> 22, 23, 24, 25, 26, 27 et 28 sont des lots de tests purs.
...
### 6.2 Fichiers de test colocalisés (43 fichiers, 873 tests)
...
Mesure après le LOT 28 (V8, `npm run test:coverage`), sur les zones ciblées :
...
| **Global `src/` (toutes zones)** | **57,8 %** | **57,8 %** | **52,1 %** | **76,7 %** |
...
> **57,8 % (LOT 28)**
> (fonctions **52,1 %**, branches **76,7 %**). Les LOT 17 et 19 corrigent
...
> FINANCE/consultation seule et état d'enregistrement.
...
**Aucune page n'est dépourvue de test.** État après le LOT 28 :
...
3. flux critiques par rôle : ~~candidatures~~ (LOT 9), ~~admissions/décisions~~
   ...
   **finance étudiante `FinancesEtudiantes` est verrouillée à 100 % au
   ...
   au LOT 28** (tarifs, tolérance, exports, habilitations). Restent
   ensuite les **autres écrans du module Finance** (`FinanceDashboard`,
   `FinanceAjustements` ; le shell/période communs et le patron de
   formulaire paramétrage sont désormais en place), les **référentiels**,
   les actions secondaires des écrans Scolarité (`Equivalences`,
   `MaquetteDetail`, `ChargesEnseignants`), et les gros volumes internes
   `Statistiques` et `Users` (création de secrétariat à la volée) ;
