...
# Accepte guillemets simples OU doubles (le manage.py de ce projet utilise les simples).
SET_MOD="$(grep -oE "DJANGO_SETTINGS_MODULE[\"']?\)?[[:space:]]*,[[:space:]]*[\"'][A-Za-z0-9_.]+" "$BACKEND/manage.py" 2>/dev/null \
           | grep -oE '[A-Za-z0-9_.]+$' | head -1)"
[ -z "$SET_MOD" ] && SET_MOD="$(grep -rhoE "DJANGO_SETTINGS_MODULE[\"']?\)?[[:space:]]*,[[:space:]]*[\"'][A-Za-z0-9_.]+" "$BACKEND" \
           --include='*.py' --exclude-dir='.venv' --exclude-dir='node_modules' 2>/dev/null \
           | grep -oE '[A-Za-z0-9_.]+$' | head -1)"
