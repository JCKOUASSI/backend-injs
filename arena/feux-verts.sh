...
if [ -n "$BACKEND" ]; then
  ...
  # Le parallélisme est désactivé par défaut dans Arena (FEUX_VERTS_PARALLEL=1) :
  # sur ce sandbox, une exception dans un worker remonte « cannot pickle
  # 'traceback' object » et masque le vrai résultat. Sur une machine CI/poste
  # équipé, exporter FEUX_VERTS_PARALLEL=2 rétablit --parallel 2.
  FV_PAR="${FEUX_VERTS_PARALLEL:-1}"
  POPT=""; [ "$FV_PAR" -gt 1 ] && POPT="--parallel $FV_PAR"
  echo "  > test (SQLite de secours, parallel=$FV_PAR)"
  if python manage.py test $OPT $POPT >>"$LOG" 2>&1; then
